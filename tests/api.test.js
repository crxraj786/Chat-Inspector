...
const port = 3217;
let child;
before(async () => { child = spawn(process.execPath, ['server/index.js'], { env: { ...process.env, PORT: String(port), CHAT_INSPECTOR_API_KEY: 'test-key' } }); await new Promise((resolve, reject) => { const timer = setTimeout(() => reject(new Error('server timeout')), 3000); child.stdout.on('data', (buf) => { if (buf.toString().includes('listening')) { clearTimeout(timer); resolve(); } }); child.on('error', reject); }); });
after(() => child?.kill());
const request = (path, options = {}) => fetch(`http://127.0.0.1:${port}${path}`, { ...options, headers: { authorization: 'Bearer test-key', 'content-type': 'application/json', ...(options.headers || {}) } });
