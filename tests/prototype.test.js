...
test('manifest does not request cookies or broad host permissions', () => { const manifest = JSON.parse(fs.readFileSync('extension/manifest.json', 'utf8')); assert.deepEqual(manifest.permissions, ['storage']); assert.ok(manifest.host_permissions.every((host) => host.includes('facebook.com/messages') || host.includes('messenger.com'))); assert.equal(manifest.background.service_worker, 'background/service-worker.js'); });
